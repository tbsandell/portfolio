...
function changeProb() {
	...
	if (e.options[e.selectedIndex].value == "All") {
		...
		for (let i = 1; i < euler.length; i++) {
			$("#euler").innerHTML += eulerHTML(i);
		}
	} else {
		$("#euler").innerHTML = eulerHTML(e.options[e.selectedIndex].value);
	}
	MathJax.typeset();
...
function eulerHTML(num) {
	...
	for (let i = 0; i < e.code.length; i++) {
		code += `<button style="display: inline-block;" onclick="$('#euler${num}Code${i}').hidden = !$('#euler${num}Code${i}').hidden">${e.code[i]}</button>`;
	}
		...
		<td class="euler-col-right col-6">
			<form onsubmit="eulerEval(${num}); return false;">
				<p style="font-family:monospace;font-size:16px;">euler${num}(<input 
					oninput="$('#euler${num}Input').size = $('#euler${num}Input').value.length;"
